...
foreach ($file in $ProtoFiles) {
    protoc --proto_path="$($file.DirectoryName)" --go_out=paths=source_relative:./golsql --go-grpc_out=paths=source_relative:./golsql --grpc-web_out=import_style=commonjs+dts,mode=grpcwebtext:./lsql.ts/src/generated $file.FullName
}
